...
static av_cold int arcdav3a_decode_init(AVCodecContext *avctx)
{
    ...
    if (!h->handle) {
#ifdef _WIN32
        h->handle = dlopen("AVS3AudioDec.dll", RTLD_LAZY);
#else
        h->handle = dlopen("libAVS3AudioDec.so", RTLD_LAZY);
    }
    if (h->handle == NULL) {
#ifdef _WIN32
        av_log(avctx, AV_LOG_ERROR, "load AVS3AudioDec.dll failed: %d\n", ferror(h->renderhandle));

#else
        av_log(avctx, AV_LOG_ERROR, "load libAVS3AudioDec.so failed: %d\n", ferror(h->renderhandle));
#endif
        return AVERROR(EFAULT);
    }
