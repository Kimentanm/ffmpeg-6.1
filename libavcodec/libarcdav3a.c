...
static int InitRender(AVCodecContext *avctx, AVS3DecoderOutput *pOut) {
    ...
    if (!h->renderhandle) {
#ifdef _WIN32
        h->renderhandle = dlopen("av3a_binaural_render.dll", RTLD_LAZY);
#else
...
#ifdef _WIN32
        av_log(avctx, AV_LOG_ERROR, "load av3a_binaural_render.dll failed: %d\n", ferror(h->renderhandle));

#else
        av_log(avctx, AV_LOG_ERROR, "load libav3a_binaural_render.so failed: %d\n", ferror(h->renderhandle));
#endif
    ...
    h->GetBinauralInterleavedAudioBuffer = (PFGetBinauralInterleavedAudioBuffer) dlsym(h->renderhandle,
                                                                                       "GetBinauralInterleavedAudioBuffer");
...
static int dav3a_decode_frame(AVCodecContext *avctx, const char *pIn, unsigned long lenIn, unsigned long *lenConsumed,
    ...
    *lenConsumed = lenIn;
    ...
    do {
        int consumed = 0;
        while ((ret = h->parse_header(h->m_hAvs3, h->m_pBuffer + pos, h->m_dwDataLen - pos, h->m_bFirstFrame, &consumed,
                                      NULL)) != AVS3_TRUE) {
        ...
        if (((h->m_lastChCfg != -1) && (h->m_lastChCfg != h->m_hAvs3->channelNumConfig))
            ...
            av_log(avctx, AV_LOG_DEBUG,
                   ...
                   h->m_hAvs3->channelNumConfig, h->m_lastObjcnt, h->m_hAvs3->numObjsOutput,
...
//		ASLOG_MsgA(5, "%s, m_dwDataLen:%d, m_hAvs3->bitsPerFrame:%d, %d", __FUNCTION__, m_dwDataLen, m_hAvs3->bitsPerFrame, m_hAvs3->bitsPerFrame/8);
        ...
        ret = h->avs3_decode(h->m_hAvs3, h->m_pBuffer + pos, h->m_dwDataLen - pos, pOut->pOutData + outindex, &outlen,
                             &consumed);
        pos += consumed;
    ...
    if (outindex) {
        ...
        if (h->m_hAvs3->hMetadataDec->avs3MetaData.hasStaticMeta ||
            h->m_hAvs3->hMetadataDec->avs3MetaData.hasDynamicMeta) {
...
//		memcpy(&metaData, pOut->pMeta, min(pOut->nMeta, sizeof(metaData)));
    ...
    while (h->m_dwRenderDataLen - pos >= 1024)//一次render 1024 个sample
    {
        ...
        if (h->PutInterleavedAudioBuffer(h->m_pRender, h->m_pRenderBuffer, 1024, h->m_nlastChannels) !=
            0) //m_pRender: handle  m_pRenderBuffer:存放要render的sample
        {
            err = 1;
            break;
        }
    ...
    *got_frame_ptr = 0;
    ...
    if (avpkt->size > 0) {
        ...
        while (size > 0)//
        {
            ...
            if (h->out_frame.pMeta &&
                memcmp(h->out_frame.pMeta, &h->m_LastMetaData, sizeof(h->m_LastMetaData)) != 0)//metadata发生变化
            {
